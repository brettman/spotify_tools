...
public class PlaybackTracker : BackgroundService
{
    ...
    private async Task EnsureAuthenticatedAsync()
    {
        ...
        _logger.LogInformation("Attempting to authenticate with Spotify using stored refresh token...");
        ...
        if (token == null)
        {
            _logger.LogError("No stored authentication token found in database.");
            _logger.LogWarning("Please run the CLI application once to authenticate first.");
            throw new InvalidOperationException(
                "No stored Spotify authentication found. Please run the CLI application to authenticate first.");
        }

        if (string.IsNullOrEmpty(token.EncryptedRefreshToken))
        {
            _logger.LogError("Stored token has no refresh token.");
            throw new InvalidOperationException(
                "Stored token is invalid. Please re-authenticate using the CLI application.");
        }
        ...
        try
        {
            ...
            _logger.LogInformation("Successfully authenticated with Spotify as user {User}", _spotifyClient.UserId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to authenticate with stored refresh token");
            throw new InvalidOperationException(
                "Failed to authenticate with stored token. The token may have expired. Please re-authenticate using the CLI application.", ex);
        }
    }
